...
class Hash {
  ...
  long long inverse(long long x, long long p) { return power(x, p - 2, p); }
  ...
  pair<int, int> prefix(const int index) { return {hash1[index], hash2[index]}; }
  ...
  bool operator==(const Hash& other) { return (hash_pair == other.hash_pair); }
};
